...
def initialisation(N=500,rho=0.84):#N=4n^3 ,N-no of particles,n-number of particles in one direction, rho-density
    n=np.round((N/4)**(1/3))
    if 4*n**3 != N:
        raise ValueError("N must be a multiple of 4*n^3 for a simple cubic lattice.")
...
run_langevin=True
plot_cords=[]
def integrate(coordinates,velocities,e,time=200):
    x=coordinates
    v=velocities
    for i in range(time):
        ...
        if run_molsim==True and run_langevin==True and run_test==False:
            h=t
            random_rotations = np.random.normal(scale=np.sqrt(2 * Dr * h), size=(N, 3))
            e=e+np.cross(random_rotations, e)
            norm=np.linalg.norm(e, axis=1,keepdims=True)
            e=e/norm
            ...
            x
            + h * v_half                        # + h v^{n+1/2}
            + h**1.5 * sigma * (1/(2*np.sqrt(3))) * eta)+v0*e*h
            x_n_1=x_n.flatten()
            F,U=molsim.forces(x_n_1,L,N,r_c)
            #print(U)
            #print(F)
...
if run_test==False:
    ...
    plt.show()
print(plot_cords)
def animate(plot_cords,interval=50):
    fig=plt.figure()#creating a blank canvas
    ax=fig.add_subplot(projection="3d")#creates a specific plotting area in the canvas
    def update(frame):
