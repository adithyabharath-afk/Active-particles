...
def integrate(coordinates,velocities,e,time=10000):
    ...
    for i in range(time):
        ...
        if i>6000 and run_test==False:
            T_m=np.mean(np.linalg.norm(v_n, axis=1)**2)/2
            T_series.append(T_m)
        coordinates=x_n
        velocities=v_n
        if run_test==False:
...
def animate(plot_cords,interval=50):
    ...
    def update(frame):
        ...
        ax.scatter(cords[:,0],cords[:,1],s=5)
        ax.set_xlim([0,L])
        ax.set_ylim([0,L])
